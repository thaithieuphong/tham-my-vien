...
const previewBeforeVideo = document.querySelector('#preview-videos-before');

const inputBeforeData = document.querySelector('#input-multi-videos-before');
const dataBeforeID = inputBeforeData.getAttribute('data-before-id');
const dataBeforeCusID = inputBeforeData.getAttribute('data-before-customer-id');

// Create a FilePond instance

const uploadBeforeVideo = FilePond.create(previewBeforeVideo, {
    name: 'before',
...
uploadBeforeVideo.setOptions({
    fileMetadataObject: {
        cusID: dataBeforeCusID,
    },
    server: {
        process: (fieldName, file, metadata, load, error, progress, abort, transfer, options) => {
            ...
            request.open('POST', `/business/employ/service-note/${dataBeforeID}/uploadBeforeVideos`);
